defmodule Erm.Core.ApplicationTest do
  ...
  test "applications are found" do
    assert List.first(applications()) == Application.find_application(applications(), "Pelikan inks")
  end
  ...
  test "action can be run from applications" do
    app = application()
    par = params()
    assert {:ok, app, par} == Application.run_action(applications(), "Pelikan inks", :dumb_action, par)
  end
