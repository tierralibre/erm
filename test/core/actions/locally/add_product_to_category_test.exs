defmodule Erm.Core.Actions.Locally.AddProductToCategoryTest do
  ...
  test "can relate product and category is added to the application" do
    ...
    {:ok, application1, %{entity: product}} = AddProduct.run(Application.new("Application", []), data)
    ...
    assert %Application{relations: [%Relation{type: :belongs_category, from: product_uuid, to: category_uuid, data: data}]} = application
  end
end
