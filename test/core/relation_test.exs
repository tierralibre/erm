defmodule Erm.Core.RelationTest do
  ...
  test "relations are listed by type" do
    ...
    entities = Relation.list_relations(app.name, app.persistence, "belongs_category", %{})
  ...
  test "relations are listed by type and to" do
    ...
    entities = Relation.list_relations(app.name, app.persistence, "belongs_category", %{to: cat.id})
    assert length(entities) == 1
    assert List.first(entities).type == "belongs_category"
  end
  ...
  test "relations are listed by type and from" do
    ...
    entities = Relation.list_relations(app.name, app.persistence, "belongs_category", %{from: pr.id})
    assert length(entities) == 1
    assert List.first(entities).type == "belongs_category"
  end
  ...
  test "relations are listed by type and from and to" do
    ...
    entities = Relation.list_relations(app.name, app.persistence, "belongs_category", %{from: pr.id, to: cat.id})
    assert length(entities) == 1
    assert List.first(entities).type == "belongs_category"
  end
  ...
  test "gets relations" do
    ...
    assert %Relation{type: "belongs_category"} =
             Relation.get_relation(app.name, app.persistence, "belongs_category", pr.id, cat.id)
  end
end
